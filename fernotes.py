"""FerNotes - Simple Tkinter note/drawing app

This file contains the runnable FerNotes application.
"""
...
def set_color(color):
    global current_color, eraser_mode, text_mode
    current_color = color
    eraser_mode = False
    text_mode = False
...
def toggle_eraser():
    global eraser_mode, text_mode
    eraser_mode = not eraser_mode
    text_mode = False


def toggle_text():
    global text_mode, eraser_mode
...
def start_draw(event):
    ...
    if text_mode:
        text = simpledialog.askstring("Add Text", "Type something:")
        if text:
...
def save_canvas():
    file = filedialog.asksaveasfilename(
        defaultextension=".fernotes", filetypes=[("FerNotes files", "*.fernotes")]
    )
    if not file:
        return

    data = {"title": title_entry.get(), "items": []}
...
def open_canvas():
    file = filedialog.askopenfilename(filetypes=[("FerNotes files", "*.fernotes")])
    if not file:
        return
    ...
    title_entry.insert(0, data.get("title", "Untitled Notes"))

    for item in data.get("items", []):
        if item.get("type") == "line":
            canvas.create_line(
                *item.get("coords", []),
                fill=item.get("fill", current_color),
                width=float(item.get("width", brush_size)),
                smooth=True,
                capstyle=tk.ROUND,
                joinstyle=tk.ROUND,
            )
            ...
            canvas.create_text(
                *item.get("coords", []),
                text=item.get("text", ""),
                fill=item.get("fill", current_color),
                font=item.get("font", ("Arial", max(8, brush_size * 4))),
            )
...
def draw_lines():
    for y in range(40, HEIGHT, 40):
        canvas.create_line(0, y, WIDTH, y, fill="#e8e1ff")
...
canvas = tk.Canvas(root, bg=BG_COLOR, highlightthickness=0, width=WIDTH, height=HEIGHT)
